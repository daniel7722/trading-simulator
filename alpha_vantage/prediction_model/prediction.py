...
def download_data(config):
    ...
    data_date = data_date[:500] # only use the first 1000 data points
    data_date.reverse()

    data_close_price = [float(data[date][config["alpha_vantage"]["key_close"]]) for date in data.keys()]
    data_close_price = data_close_price[:500] # only use the first 1000 data points
